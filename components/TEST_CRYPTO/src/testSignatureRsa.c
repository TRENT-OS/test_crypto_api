...
static const SeosCryptoKey_RSAPrv clientRSAPrvData =
{
    .nBytes = {
...
static const SeosCryptoKey_RSAPub serverRSAPubData =
{
    .nBytes = {
        0x92, 0x22, 0xa2, 0x1b, 0x01, 0x61, 0xff, 0xc3, 0xdd, 0xc0, 0x4f, 0x8e, 0x91, 0xf1, 0xcc, 0x1f,
        0xdc, 0x0d, 0x2a, 0x08, 0x66, 0xaf, 0x0d, 0xd9, 0x05, 0xe8, 0xe7, 0xd6, 0x52, 0xa0, 0x38, 0x62,
...
testSignatureRSA(SeosCryptoCtx* cryptoCtx)
{
    ...
    err = SeosCryptoApi_keyInit(cryptoCtx, &pubHandle,
                                SeosCryptoKey_Type_RSA_PUB, 0, 1024);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_keyInit(cryptoCtx, &prvHandle,
                                SeosCryptoKey_Type_RSA_PRV, 0, 1024);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    err = SeosCryptoApi_keyImport(cryptoCtx, pubHandle, NULL, &serverRSAPubData,
                                  sizeof(serverRSAPubData));
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_keyImport(cryptoCtx, prvHandle, NULL, &clientRSAPrvData,
                                  sizeof(clientRSAPrvData));
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
