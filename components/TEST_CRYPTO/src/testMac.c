       ...
       const macTestVector*     vec)
{
    ...
    err = SeosCryptoApi_macProcess(ctx, macHandle, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    macSize = sizeof(mac);
    err = SeosCryptoApi_macFinalize(ctx, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
...
static void
testMac_start_fail(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
    ...
    // Empty ctx
    err = SeosCryptoApi_macStart(NULL, macHandle, vec->secret.bytes,
                                 vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Empty handle
    err = SeosCryptoApi_macStart(ctx, NULL, vec->secret.bytes, vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_HANDLE == err, "err %d", err);

    // Empty secret buf
    err = SeosCryptoApi_macStart(ctx, macHandle, NULL, vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Zero-len secret
    err = SeosCryptoApi_macStart(ctx, macHandle, vec->secret.bytes, 0);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);
...
static void
testMac_process_fail(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
    const macTestVector* vec = &md5Vectors[0];
    ...
    // Test with empty ctx
    err = SeosCryptoApi_macProcess(NULL, macHandle, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Test with empty handle
    err = SeosCryptoApi_macProcess(ctx, NULL, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_HANDLE == err, "err %d", err);

    // Test with empty input
    err = SeosCryptoApi_macProcess(ctx, macHandle, NULL, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Test with zero lenght input
    err = SeosCryptoApi_macProcess(ctx, macHandle, vec->msg.bytes, 0);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);
...
static void
testMac_finalize_fail(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
    const macTestVector* vec = &md5Vectors[0];
    ...
    err = SeosCryptoApi_macFinalize(NULL, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Finalize without handle
    err = SeosCryptoApi_macFinalize(ctx, NULL, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_HANDLE == err, "err %d", err);

    // Finalize without output buffer
    err = SeosCryptoApi_macFinalize(ctx, macHandle, NULL, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);
...
static void
testMac_start_buffer(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
    ...
    err = SeosCryptoApi_macStart(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
...
static void
testMac_process_buffer(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
    static unsigned char inBuf[SeosCrypto_Size_DATAPORT + 1];
    ...
    err = SeosCryptoApi_macInit(ctx, &macHandle, SeosCryptoMac_Algorithm_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_macStart(ctx, macHandle, inBuf, 16);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Should go OK
    inLen = SeosCrypto_Size_DATAPORT;
    err = SeosCryptoApi_macProcess(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
...
static void
testMac_finalize_buffer(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
    ...
    err = SeosCryptoApi_macFinalize(ctx, macHandle, outBuf, &outLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_macFree(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    ...
    err = SeosCryptoApi_macFinalize(ctx, macHandle, outBuf, &outLen);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_BUFFER_TOO_SMALL == err, "err %d", err);
    Debug_ASSERT(outLen == SeosCryptoMac_Size_HMAC_MD5);
    err = SeosCryptoApi_macFree(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
...
static void
testMac_init_ok(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
...
static void
testMac_init_fail(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;

    // Empty context
    err = SeosCryptoApi_macInit(NULL, &macHandle, SeosCryptoMac_Algorithm_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Empty handle
    err = SeosCryptoApi_macInit(ctx, NULL, SeosCryptoMac_Algorithm_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Incorrect algorithm
    err = SeosCryptoApi_macInit(ctx, &macHandle, 666);
...
static void
testMac_free_ok(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
...
static void
testMac_free_fail(SeosCryptoCtx* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCrypto_MacHandle macHandle;
    ...
    // Empty ctx
    err = SeosCryptoApi_macFree(NULL, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Empty handle
    err = SeosCryptoApi_macFree(ctx, NULL);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_HANDLE == err, "err %d", err);
...
void
testMac(SeosCryptoCtx* ctx)
{
    testMac_init_ok(ctx);
    testMac_init_fail(ctx);

    testMac_free_ok(ctx);
    testMac_free_fail(ctx);

    testMac_mac_HMAC_MD5(ctx);
    testMac_mac_HMAC_SHA256(ctx);

    // Test only failures separately, as computing ref. values is sufficient
    // proof of correct funtioning
    testMac_start_fail(ctx);
    testMac_process_fail(ctx);
    testMac_finalize_fail(ctx);

    testMac_start_buffer(ctx);
    testMac_process_buffer(ctx);
    testMac_finalize_buffer(ctx);
}