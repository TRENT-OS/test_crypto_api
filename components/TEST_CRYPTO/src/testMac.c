/**
 * Copyright (C) 2019, Hensoldt Cyber GmbH
 *
 */

#include "SeosCryptoApi.h"

#include "LibDebug/Debug.h"

#include <string.h>

#define MAX_VECTOR_SIZE 2048
typedef struct
{
    size_t len;
    unsigned char bytes[MAX_VECTOR_SIZE];
} vector_t;

typedef struct
{
    vector_t secret;
    vector_t msg;
    vector_t mac;
} macTestVector;

// -----------------------------------------------------------------------------

#define NUM_MD5_TESTS 3
static const macTestVector md5Vectors[NUM_MD5_TESTS] =
{
    {
        .secret  = {
            .bytes = {0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61},
            .len = 16
        },
        .msg = {
            .bytes = {0xb9, 0x1c, 0xe5, 0xac, 0x77, 0xd3, 0x3c, 0x23, 0x4e, 0x61, 0x00, 0x2e, 0xd6},
            .len = 13
        },
        .mac = {
            .bytes = {0x42, 0x55, 0x28, 0x82, 0xf0, 0x0b, 0xd4, 0x63, 0x3e, 0xa8, 0x11, 0x35, 0xa1, 0x84, 0xb2, 0x84},
            .len = 16
        },
    },
    {
        .secret  = {
            .bytes = {0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61},
            .len = 16
        },
        .msg = {
            .bytes = {},
            .len = 153
        },
        .mac = {
            .bytes = {0xa1, 0x6a, 0x84, 0x28, 0x91, 0x78, 0x6d, 0x01, 0xfe, 0x50, 0xba, 0x77, 0x31, 0xdb, 0x74, 0x64},
            .len = 16
        },
    },
    {
        .secret  = {
            .bytes = {0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61},
            .len = 70
        },
        .msg = {
            .bytes = {0xb9, 0x1c, 0xe5, 0xac, 0x77, 0xd3, 0x3c, 0x23, 0x4e, 0x61, 0x00, 0x2e, 0xd6},
            .len = 13
        },
        .mac = {
            .bytes = {0xe9, 0x7f, 0x62, 0x39, 0x36, 0xf9, 0x8a, 0x7f, 0x74, 0x1c, 0x4b, 0xd0, 0x61, 0x2f, 0xec, 0xc2},
            .len = 16
        },
    },
};

// -----------------------------------------------------------------------------

#define NUM_SHA256_TESTS 3
static const macTestVector sha256Vectors[NUM_SHA256_TESTS] =
{
    {
        .secret  = {
            .bytes = {0xcf, 0xd4, 0xa4, 0x49, 0x10, 0xc9, 0xe5, 0x67, 0x50, 0x7a, 0xbb, 0x6c, 0xed, 0xe4, 0xfe, 0x60, 0x1a, 0x7a, 0x27, 0x65, 0xc9, 0x75, 0x5a, 0xa2, 0xcf, 0x6b, 0xa4, 0x81, 0x42, 0x23, 0x81, 0x1a, 0x26, 0xa8, 0xa1, 0xef, 0x49, 0x9c, 0xeb, 0xd9},
            .len = 40
        },
        .msg = {
            .bytes = {},
            .len = 128
        },
        .mac = {
            .bytes = {0x20, 0x15, 0x3b, 0xf8, 0xea, 0x29, 0x53, 0xc4, 0x82, 0x51, 0xeb, 0xcc, 0x41, 0x61, 0xf8, 0xb6, 0xe2, 0x84, 0x99, 0xe5, 0xc7, 0x6c, 0x24, 0x01, 0x4c, 0xff, 0x4a, 0x9e, 0x2f, 0x62, 0xd2, 0x5c},
            .len = 32
        },
    },
    {
        .secret  = {
            .bytes = {0x54, 0x48, 0x99, 0x8f, 0x9d, 0x8f, 0x98, 0x53, 0x4a, 0xdd, 0xf0, 0xc8, 0xba, 0x63, 0x1c, 0x49, 0x6b, 0xf8, 0xa8, 0x00, 0x6c, 0xbb, 0x46, 0xad, 0x15, 0xfa, 0x1f, 0xa2, 0xf5, 0x53, 0x67, 0x12, 0x0c, 0x19, 0x34, 0x8c, 0x3a, 0xfa, 0x90, 0xc3},
            .len = 40
        },
        .msg = {
            .bytes = {},
            .len = 128
        },
        .mac = {
            .bytes = {0x7e, 0x8c, 0xba, 0x9d, 0xd9, 0xf0, 0x6e, 0xbd, 0xd7, 0xf9, 0x2e, 0x0f, 0x1a, 0x67, 0xc7, 0xf4, 0xdf, 0x52, 0x69, 0x3c, 0x21, 0x2b, 0xdd, 0x84, 0xf6, 0x73, 0x70, 0xb3, 0x51, 0x53, 0x3c, 0x6c},
            .len = 32
        },
    },
    {
        .secret  = {
            .bytes = {0x9d, 0xa0, 0xc1, 0x14, 0x68, 0x2f, 0x82, 0xc1, 0xd1, 0xe9, 0xb5, 0x44, 0x30, 0x58, 0x0b, 0x9c, 0x56, 0x94, 0x89, 0xca, 0x16, 0xb9, 0x2e, 0xe1, 0x04, 0x98, 0xd5, 0x5d, 0x7c, 0xad, 0x5d, 0xb5, 0xe6, 0x52, 0x06, 0x34, 0x39, 0x31, 0x1e, 0x04},
            .len = 40
        },
        .msg = {
            .bytes = {},
            .len = 128
        },
        .mac = {
            .bytes = {0xcd, 0xea, 0xcf, 0xce, 0xbf, 0x46, 0xcc, 0x9d, 0x7e, 0x4d, 0x41, 0x75, 0xe5, 0xd8, 0xd2, 0x67, 0xc2, 0x3a, 0x64, 0xcd, 0xe8, 0x3e, 0x86, 0x7e, 0x50, 0x01, 0xec, 0xf2, 0x6f, 0xbd, 0x30, 0xd2},
            .len = 32
        },
    },
};

// -----------------------------------------------------------------------------

static void
do_mac(SeosCryptoApi_Context* ctx,
       SeosCryptoApi_Mac      macHandle,
       const macTestVector*   vec)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    char mac[64];
    size_t macSize;

    err = SeosCryptoApi_Mac_start(ctx, macHandle, vec->secret.bytes,
                                 vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    err = SeosCryptoApi_Mac_process(ctx, macHandle, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    macSize = sizeof(mac);
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    Debug_ASSERT(macSize == vec->mac.len);
    Debug_ASSERT(!memcmp(mac, vec->mac.bytes, vec->mac.len));
}

static void
testMac_mac_HMAC_MD5(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;
    size_t i;

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    for (i = 0; i < NUM_MD5_TESTS; i++)
    {
        do_mac(ctx, macHandle, &md5Vectors[i]);
    }

    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_mac_HMAC_SHA256(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;
    size_t i;

    err = SeosCryptoApi_Mac_init(ctx, &macHandle,
                                SeosCryptoApi_Mac_ALG_HMAC_SHA256);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    for (i = 0; i < NUM_SHA256_TESTS; i++)
    {
        do_mac(ctx, macHandle, &sha256Vectors[i]);
    }

    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_start_fail(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;
    const macTestVector* vec = &md5Vectors[0];

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Empty ctx
    err = SeosCryptoApi_Mac_start(NULL, macHandle, vec->secret.bytes,
                                 vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Empty handle
    err = SeosCryptoApi_Mac_start(ctx, NULL, vec->secret.bytes, vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_HANDLE == err, "err %d", err);

    // Empty secret buf
    err = SeosCryptoApi_Mac_start(ctx, macHandle, NULL, vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Zero-len secret
    err = SeosCryptoApi_Mac_start(ctx, macHandle, vec->secret.bytes, 0);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Start after already having started it
    err = SeosCryptoApi_Mac_start(ctx, macHandle, vec->secret.bytes,
                                 vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_start(ctx, macHandle, vec->secret.bytes,
                                 vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_ABORTED == err, "err %d", err);

    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_process_fail(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;
    const macTestVector* vec = &md5Vectors[0];
    char mac[64];
    size_t macSize = sizeof(mac);

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_start(ctx, macHandle, vec->secret.bytes,
                                 vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Test with empty ctx
    err = SeosCryptoApi_Mac_process(NULL, macHandle, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Test with empty handle
    err = SeosCryptoApi_Mac_process(ctx, NULL, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_HANDLE == err, "err %d", err);

    // Test with empty input
    err = SeosCryptoApi_Mac_process(ctx, macHandle, NULL, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Test with zero lenght input
    err = SeosCryptoApi_Mac_process(ctx, macHandle, vec->msg.bytes, 0);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Process after already finalizing it
    err = SeosCryptoApi_Mac_process(ctx, macHandle, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_process(ctx, macHandle, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_ABORTED == err, "err %d", err);

    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_finalize_fail(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;
    const macTestVector* vec = &md5Vectors[0];
    char mac[64];
    size_t macSize = sizeof(mac);

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Finalize without updating
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_ABORTED == err, "err %d", err);

    err = SeosCryptoApi_Mac_start(ctx, macHandle, vec->secret.bytes,
                                 vec->secret.len);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_process(ctx, macHandle, vec->msg.bytes, vec->msg.len);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Finalize without context
    err = SeosCryptoApi_Mac_finalize(NULL, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Finalize without handle
    err = SeosCryptoApi_Mac_finalize(ctx, NULL, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_HANDLE == err, "err %d", err);

    // Finalize without output buffer
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, NULL, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Finalize without sufficient space
    macSize = 4;
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_BUFFER_TOO_SMALL == err, "err %d", err);

    // Finalize twice
    macSize = sizeof(mac);
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, mac, &macSize);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_ABORTED == err, "err %d", err);

    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_start_buffer(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;
    static unsigned char inBuf[SeosCryptoApi_SIZE_DATAPORT + 1];
    size_t inLen;

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Should go OK
    inLen = SeosCryptoApi_SIZE_DATAPORT;
    err = SeosCryptoApi_Mac_start(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Should fail due to internal buffers being limited
    inLen = SeosCryptoApi_SIZE_DATAPORT + 1;
    err = SeosCryptoApi_Mac_start(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INSUFFICIENT_SPACE == err, "err %d", err);

    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_process_buffer(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;
    static unsigned char inBuf[SeosCryptoApi_SIZE_DATAPORT + 1];
    size_t inLen;

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_start(ctx, macHandle, inBuf, 16);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Should go OK
    inLen = SeosCryptoApi_SIZE_DATAPORT;
    err = SeosCryptoApi_Mac_process(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Should fail due to internal buffers being limited
    inLen = SeosCryptoApi_SIZE_DATAPORT + 1;
    err = SeosCryptoApi_Mac_process(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INSUFFICIENT_SPACE == err, "err %d", err);

    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_finalize_buffer(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;
    static unsigned char inBuf[SeosCryptoApi_SIZE_DATAPORT],
                         outBuf[SeosCryptoApi_SIZE_DATAPORT + 1];
    size_t inLen, outLen;

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    inLen = SeosCryptoApi_SIZE_DATAPORT;
    err = SeosCryptoApi_Mac_start(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_process(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    // Should be OK, as we are below the dataport limit
    outLen = SeosCryptoApi_SIZE_DATAPORT;
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, outBuf, &outLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    inLen = SeosCryptoApi_SIZE_DATAPORT;
    err = SeosCryptoApi_Mac_start(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_process(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    // Should fail because out buffer is potentially too big
    outLen = SeosCryptoApi_SIZE_DATAPORT + 1;
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, outBuf, &outLen);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INSUFFICIENT_SPACE == err, "err %d", err);
    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    inLen = SeosCryptoApi_SIZE_DATAPORT;
    err = SeosCryptoApi_Mac_start(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_process(ctx, macHandle, inBuf, inLen);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    // This should fail but give us the expected buffer size
    outLen = 10;
    err = SeosCryptoApi_Mac_finalize(ctx, macHandle, outBuf, &outLen);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_BUFFER_TOO_SMALL == err, "err %d", err);
    Debug_ASSERT(outLen == SeosCryptoApi_Mac_SIZE_HMAC_MD5);
    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_init_ok(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;

    // Test HMAC_MD5
    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Test HMAC_SHA256
    err = SeosCryptoApi_Mac_init(ctx, &macHandle,
                                SeosCryptoApi_Mac_ALG_HMAC_SHA256);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_init_fail(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;

    // Empty context
    err = SeosCryptoApi_Mac_init(NULL, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Empty handle
    err = SeosCryptoApi_Mac_init(ctx, NULL, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Incorrect algorithm
    err = SeosCryptoApi_Mac_init(ctx, &macHandle, 666);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_NOT_SUPPORTED == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_free_ok(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

static void
testMac_free_fail(SeosCryptoApi_Context* ctx)
{
    seos_err_t err = SEOS_ERROR_GENERIC;
    SeosCryptoApi_Mac macHandle;

    err = SeosCryptoApi_Mac_init(ctx, &macHandle, SeosCryptoApi_Mac_ALG_HMAC_MD5);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    // Empty ctx
    err = SeosCryptoApi_Mac_free(NULL, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Empty handle
    err = SeosCryptoApi_Mac_free(ctx, NULL);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_HANDLE == err, "err %d", err);

    err = SeosCryptoApi_Mac_free(ctx, macHandle);
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);

    Debug_PRINTF("->%s: OK\n", __func__);
}

void
testMac(SeosCryptoApi_Context* ctx)
{
    testMac_init_ok(ctx);
    testMac_init_fail(ctx);

    testMac_free_ok(ctx);
    testMac_free_fail(ctx);

    testMac_mac_HMAC_MD5(ctx);
    testMac_mac_HMAC_SHA256(ctx);

    // Test only failures separately, as computing ref. values is sufficient
    // proof of correct funtioning
    testMac_start_fail(ctx);
    testMac_process_fail(ctx);
    testMac_finalize_fail(ctx);

    testMac_start_buffer(ctx);
    testMac_process_buffer(ctx);
    testMac_finalize_buffer(ctx);
}