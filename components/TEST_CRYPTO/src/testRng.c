...
testRng_getBytes_ok(SeosCryptoApi_Context* ctx)
{
    ...
    err = SeosCryptoApi_Rng_getBytes(ctx,  0, data, sizeof(data));
...
testRng_getBytes_fail(SeosCryptoApi_Context* ctx)
{
    ...
    // Invalid flag
    err = SeosCryptoApi_Rng_getBytes(ctx, 666, data, sizeof(data));
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_NOT_SUPPORTED == err, "err %d", err);

    // Empty buffer
    err = SeosCryptoApi_Rng_getBytes(ctx, 0, NULL, sizeof(data));
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    // Zero-length buffer
    err = SeosCryptoApi_Rng_getBytes(ctx, 0, data, 0);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);
...
testRng_reSeed_ok(SeosCryptoApi_Context* ctx)
{
    ...
    err = SeosCryptoApi_Rng_reseed(ctx, seed, sizeof(seed));
    Debug_ASSERT_PRINTFLN(SEOS_SUCCESS == err, "err %d", err);
...
testRng_reSeed_fail(SeosCryptoApi_Context* ctx)
{
    ...
    err = SeosCryptoApi_Rng_reseed(ctx, NULL, sizeof(seed));
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);

    err = SeosCryptoApi_Rng_reseed(ctx, seed, 0);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_INVALID_PARAMETER == err, "err %d", err);
...
testRng_reSeed_buffer(SeosCryptoApi_Context* ctx)
{
    ...
    err = SeosCryptoApi_Rng_reseed(ctx, seedBuf, seedLen);
...
testRng_getBytes_buffer(SeosCryptoApi_Context* ctx)
{
    ...
    err = SeosCryptoApi_Rng_getBytes(ctx, 0, rngBuf, rngLen);
    Debug_ASSERT_PRINTFLN(SEOS_ERROR_ABORTED == err, "err %d", err);
